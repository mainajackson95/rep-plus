...
export function initMultiTabCapture() {
    ...
    // Feature-detect runtime permissions API (not supported in Firefox)
    const supportsRuntimePermissions = !!(chrome.permissions && chrome.permissions.request);

    if (supportsRuntimePermissions) {
        // Chromium-style runtime permissions
        chrome.permissions.contains({
            permissions: ['webRequest'],
            origins: ['<all_urls>']
        }, (result) => {
        ...
        // Firefox-style: emulate a toggle using storage, no real permission change
    ...
    if (multiTabBtn) {
        multiTabBtn.addEventListener('click', () => {
            if (!supportsRuntimePermissions) {
                // Emulate toggle via storage and connect/disconnect
                chrome.storage.local.get({ multiTabEnabled: true }, (data) => {
